...
fn main()
{
    ...
    let serialized_data = std::fs::read("client_key_ODM.txt").expect("Failed to open client_key_ODM.txt");
    ...
    let serialized_data = std::fs::read("final_result_ODM.txt").expect("Failed to open final_result_ODM.txt");
    let mut cursor = Cursor::new(&serialized_data);
    ...
    println!("Time taken to decrypt the search result: {:?}", duration/1000);
